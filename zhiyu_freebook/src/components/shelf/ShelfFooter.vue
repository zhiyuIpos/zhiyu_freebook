<template>
  <div class="shelf-footer" v-show="isEditMode">
      ...
      <div class="shelf-footer-tab"
        ...
        <!-- v-if="item.index!==3 添加到....按钮是否显示 -->
        <div class="tab-text" :class="{'remove-text':item.index===4}" v-if="item.index !==3">{{label(item)}}</div>
      </div>
      </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      downloadBook(book){
        ...
        return new Promise((resolve,reject)=>{
          download(book,(book)=>{
            ...
            const progress = Math.floor(progressEvent.loaded
      ...
      setPrivate(){
        ...
        this.onComplete()
        if(isPrivate){
          this.simpleToast(this.$t('shelf.setPrivateSuccess')) //8-13 8:25
        }else{ 
      ...
      showPrivate(){
        this.popupMenu=this.popup({
          ...
          this.$t('shelf.closePrivateTitle') : this.$t('shelf.setPrivateTitle'),
          btn:[
            {
      ...
      showDownload() {
        this.popupMenu=this.popup({
          ...
          this.$t('shelf.removeDownloadTitle') : this.$t('shelf.setDownloadTitle'),
          btn:[
            {
